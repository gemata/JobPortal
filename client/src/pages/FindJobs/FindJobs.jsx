...
const FindJobs = ({ userData }) => {
  ...
  const openJob = (job) => {
    ...
    fetch(
      `http://localhost:5000/api/companyprofiles/company/${job.Company?.ID}`
    )
      .then((response) => response.json())
      .then((data) => {
        if (data) {
  ...
  return (
    <>
            ...
            <div className="flex justify-between pt-[0px] pb-[0px]">
              <div className="flex flex-col gap-3 w-full items-center justify-center py-10">
                ...
                <p className="paragraph w-2/3 text-center">
                  Discover exciting opportunities in the tech industry. From
                  programming and AI to design and consulting, explore openings
                  to shape the future of innovation. Take the next step towards
                  a rewarding career.
                </p>
              </div>
            </div>
      ...
      <div className={jobOpened ? "bg-gray-100" : ""}>
        <div className="flex max-w-[1500px] mx-auto ">
          <div
            className={`border transition-all bg-gray-50 ${
              jobOpened ? "w-0 hidden md:block md:w-1/3" : "w-full"
            }`}
          >
            <FindJobsSearchbar
	...
	C236.686,77.195,229.638,83.529,221.132,87.019z"
                    ...
                    <div className="flex items-center space-x-3">
                      <img
                        src={
                          jobPost.Company?.CompanyLogo
                            ? `http://localhost:5000/companyLogos/${jobPost.Company?.CompanyLogo.s3Key}`
                            : grayLogo
                        }
                ...
                <h1 className="text-center text-2xl font-semibold w-full my-10">
                  No jobs found!
                </h1>
              )}
            </div>
            ...
            {totalPages.length !== 0 ? (
              ...
              >
                {currentPage === 1 ? (
                  <button
                    disabled
                    className="border border-gray-200 p-3.5 rounded-full bg-gray-100 text-gray-400"
                  >
                    <FaArrowLeft />
                  </button>
                ) : (
                  <button
                    ...
                    className="border border-gray-300 p-3.5 rounded-full bg-gray-100 hover:bg-gray-300"
                  >
                    <FaArrowLeft />
                  </button>
                )}
                <div id="slider" className="flex items-center gap-3">
                  {totalPages.map((page) => (
                    <button
                      ...
                      className={`border ${
                        page === currentPage
                          ? "border-fuchsia-700 bg-jobportal-pink text-white"
          ...
          <div
            className={`${
              jobOpened ? "w-full md:w-2/3 pb-3" : "w-0 hidden"
            } bg-gray-50 shadow-lg relative border transition-all flex relative`}
          >
            {currentJob ? (
              <>
                <div className="flex flex-col w-full gap-3">
                  <div className="sticky top-0 bg-white flex flex-col w-full gap-3 shadow-lg px-10 py-6">
                    ...
                    <h1 className="text-2xl font-semibold">
                      {currentJob["Job Position"]?.position}
                    </h1>
                    <hr className="w-full border border-1" />
                    <Link
                      to={`/company/show/${currentJob?.CompanyID}`}
                      className="w-fit p-2"
                    >
                      <div className="flex items-center space-x-3">
                        <img
                          src={
                            currentJob.Company?.CompanyLogo
                              ? `http://localhost:5000/companyLogos/${currentJob.Company?.CompanyLogo.s3Key}`
                              : grayLogo
                          }
                          className="w-10 h-10  select-none object-cover object-center rounded-full border border-gray-300"
                        />
                        <div className="text-sm">
                          <div className="font-medium select-none underline">
                    ...
                    <div>
                      ${currentJob.salary_from} - ${currentJob.salary_to} per
                      hour
                    </div>
                    {errorMessage ? (
                        ...
                        applied ? (
                          ...
                          <>
                            <button
                              ...
                              className="bg-jobportal-pink hover:bg-fuchsia-700 text-white rounded-lg font-semibold flex items-center"
                            >
                              <div className="flex gap-2 pr-5 pl-5 pt-2 pb-2 items-center">
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                            ...
                            {saved ? (
                              <button
                                onClick={() => removeSavedJob(currentJob.ID)}
                                className={`bg-blue-500 text-white p-2 rounded ${animationClass}`}
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  fill="white"
                              ...
                              <button
                                ...
                                className={`bg-gray-400 text-white p-2 rounded ${animationClass}`}
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  fill="none"
                            ...
                            {liked ? (
                              <button
                                onClick={() => removeLikedJob(currentJob.ID)}
                                className={`bg-green-500 text-white p-2 rounded ${animationClass}`}
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  viewBox="0 0 24 24"
                  ...
                  <div className="px-10 py-5">
                    ...
                    <h3 className="mt-3 ml-3 text-lg">
                  ...
                  <div className="px-10 py-5 flex gap-3 flex-col">
                    ...
                    <h3 className="text-lg mt-3 ml-3">
                      ...
                      {currentCompany?.address || "None specified"}
                    </h3>
                    <h3 className="text-lg mt-3 ml-3">
                      <span className="font-semibold mr-2">
                        Company Email:{" "}
                      </span>{" "}
                      {currentCompany?.companyEmail || "None specified"}
                    </h3>
                    <h3 className="text-lg mt-3 ml-3">
                      <span className="font-semibold mr-2">Phone Number: </span>{" "}
                      {currentCompany?.phoneNumber || "None specified"}
                    </h3>
                    <h3 className="text-lg mt-3 ml-3">
                      <span className="font-semibold mr-2">Website: </span>{" "}
                      {currentCompany?.website || "None specified"}
                    </h3>
                    <h3 className="text-lg mt-3 ml-3">
                      <span className="font-semibold mr-2">Description: </span>{" "}
                      {currentCompany?.description || "None specified"}
                    </h3>
                  </div>
                  <hr />
                  <div className="px-10 py-5 flex gap-3 flex-col">
