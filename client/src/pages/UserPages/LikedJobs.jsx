                          ...
                          <div className='flex justify-between items-center w-full mr-6 relative'>
                            ...
                            >
                              <div className='flex items-center  gap-10 w-full pl-5 pr-5'>
                                <p className='text-gray-600 font-bold w-1/24'>{LikedJob.JobPost.ID}</p>
                                <div className='text-left w-3/12'>
                                  <h5 className='text-jobportal-primary font-bold'>{LikedJob.JobPost.positionName}</h5>
                                  <p className='text-gray-600'>{LikedJob.JobPost.jobLocation}</p>
                                  <p className='text-gray-600'>Posted: {formatDate(LikedJob.JobPost.createdAt)}</p>
                                </div>

                                <p className='text-gray-600 w-1/12 '>{formatDate(LikedJob.JobPost.startAt)}</p>
                                <p className='text-gray-600 w-1/12 '>{formatDate(LikedJob.JobPost.endAt)}</p>
                                <p className='text-gray-600 w-2/12 '>{LikedJob.JobPost.salary_from + '$ - ' + LikedJob.JobPost.salary_to + '$'}</p>
                                <p className='text-gray-600 w-2/12 capitalize'>{LikedJob.JobPost.interviewMethod}</p>
                                <p className='text-gray-600 w-1/12 capitalize'>{LikedJob.status}</p>
                              </div>
                            </Link>
                            <button
                              onClick={() => {
