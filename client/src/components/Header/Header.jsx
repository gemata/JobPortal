...
const Header = ({ userData, setUserData, isLoggedOut, setIsLoggedOut }) => {
  ...
  const getLinkClassName = (path) => {
    return location.pathname === path ? 'text-blue-900' : 'hover:underline hover:underline-offset-4 hover:decoration-2 hover:decoration-blue-900 cursor-pointer';
  };
      ...
      <div className='p-[5px] bg-[#f1f2f4]'>
        <div className='max-w-[1200px] mx-auto my-0 px-[15px] py-0'>
          <ul className='flex items-center gap-10'>
            ...
            <Link to='/upload-resume' className={getLinkClassName('/upload-resume')}>
              Upload Resume
            </Link>
          </ul>
          <div className='flex justify-end mt-[-25px]'>
            <Link to='/company/dashboard' className='hover:underline hover:underline-offset-4 hover:decoration-2 hover:decoration-blue-900 cursor-pointer'>
              Employers/Post Job
            </Link>{' '}
