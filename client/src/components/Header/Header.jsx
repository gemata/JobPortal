...
const Header = ({ userData, setUserData, isLoggedOut, setIsLoggedOut }) => {
  ...
  return (
    <>
      <div className={`${location.pathname !== '/find-jobs' ? 'md:sticky md:top-0' : ''} bg-[#B051AA]`} style={{ zIndex: 200 }}>
        <div className='p-[15px]'>
          <div className='max-w-[1200px] mx-auto my-0 px-[15px] py-0 flex items-center justify-between'>
            ...
            <div className='md:hidden'>
              ...
              {isMenuOpen && (
                <div className='absolute right-0 mt-2 w-full gap-5 bg-[#B051AA] rounded-lg shadow-lg' style={{ zIndex: 300 }}>
                  <div className='flex flex-col ml-6 p-2 space-y-2'>
                    ...
                    {userData.length !== 0 && userData.role === 'Company' && (
                      <Link to='/company/dashboard' className='block pb-2 text-white hover:bg-fuchsia-700 rounded-t-lg' onClick={toggleMenu}>
                        Employers/Post Job
                      </Link>
                    )}

                    {userData.length !== 0 && userData.role === 'Admin' && (
                      <a href='http://localhost:5000/admin' className='block pb-2 text-white hover:bg-fuchsia-700 rounded-t-lg' onClick={toggleMenu}>
                        Admin Page
                      </a>
                    )}
                  </div>
